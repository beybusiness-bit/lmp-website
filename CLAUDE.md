...
- **authMode 구현 피드백 미반영**: 이번 세션에 구현한 5단계 authMode 기능의 테스트 피드백을 다음 세션에서 받을 예정
...
**🐛 버그 2: 유튜브 재생목록 '곡 더 추가하기' 클릭 시 프로젝트 프레임 사라짐**
- 재현: 유튜브 재생목록 도구에서 곡 제출 완료 후 "곡 더 추가하기" 버튼을 클릭하면, 도구 단독 URL로 직접 이동하면서 프로젝트 플레이어 프레임(뒤로가기 버튼, 상단 고정바, 스테이지 UI 등)이 없이 재생목록 도구만 단독으로 표시됨
- 원인 추정: `location.href`나 `window.open`으로 도구 URL에 직접 이동하는 코드가 있을 것
- 수정 방향: "곡 더 추가하기"는 현재 iframe 내부에서 도구를 초기 상태로 리셋(검색 탭으로 돌아가기)하는 방식으로 구현해야 함. 새 URL 이동 금지. `tools/youtube-playlist/index.html` 내에서 제출 완료 후 상태를 초기화하는 코드로 수정

**🐛 버그 3: 사이드바 활동 내역에 유튜브 재생목록 곡 제출 내역 누락**
- 재현: 사용자가 유튜브 재생목록 도구에서 곡을 제출해도, 프로젝트 플레이어 사이드바의 "나의 활동" 탭에 해당 제출 내역이 표시되지 않음
- 이전 요구사항: "체크박스 체크를 제외한 모든 활동이 나와야 한다"고 요청했음
- 수정 방향: 사이드바 활동 내역 로딩 함수에서 `cms_youtube_submissions/{projectId}/songs` 컬렉션도 조회하여 표시. 표시 형식: 곡 제목 + 아티스트 + 제출 날짜
...
2. 유튜브 재생목록 '곡 더 추가하기' 후 프레임 사라지는 버그

**🟡 중요 수정:**
3. 도구 admin 텍스트 편집기 줄바꿈 미표시 (전 도구 확인)
4. 사이드바 활동 내역 유튜브 제출 누락
5. 스테이지 제목 정렬 미적용

**🟢 기능 추가:**
6. 유튜브 재생목록 기제출 음악 내역 표시
7. 도구 admin RTE 이미지 업로드 개선 (가능 여부 먼저 판단)
8. 반응형 레이아웃 전략 답변 및 개선
