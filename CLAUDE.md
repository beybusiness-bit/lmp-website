...
**⚠️ 최우선: 기능 동작 확인 (브랜치 머지 후 유실/복원 이슈 있었음)**

세션 시작하면 바로 아래를 테스트:

1. **관리자 페이지 (`/admin/`)에서 프로젝트 열기**
   - 디자인 탭 → 키컬러 목록이 보이는지 (색상 추가/수정 가능한지)
   - 저장+배포 버튼이 삭제 버튼과 같은 높이인지
   - 미리보기 +/- 버튼이 나오는지

2. **스테이지 내용 편집**
   - 스테이지 섹션에서 "✏️ 내용 편집" 버튼이 보이는지
   - 클릭 시 편집 화면이 열리는지
   - 블록 추가 → 텍스트/이미지/혼합 잘 작동하는지
   - 포맷바에서 색상 버튼이 프로젝트 키컬러로 나오는지
   - 블록 배경색 피커에 키컬러 스와치가 나오는지
   - 블록 마진(상하/좌우) 입력·저장 되는지
   - 복제 버튼 → 다른 스테이지에서 블록 가져오기 되는지

3. **미복원 항목 (다음 세션에서 수동 적용)**
   ```
   파일: gmbf/po-c/index.html
   변경: #admin-link { display: none; }
   →  #admin-link { position: absolute; bottom: 16px; right: 20px; z-index: 20;
                    font-size: 11px; color: rgba(255,255,255,0.4);
                    cursor: pointer; -webkit-tap-highlight-color: transparent; padding: 8px; }
   ```

**유실 원인 (다음 세션 Claude에게):**
이전 세션에서 기능 브랜치(`claude/seller-portal-cms-pages-bV4Br`)를 main에 머지할 때 충돌이 발생했고, 충돌 해결 과정에서 기능 브랜치 파일 전체를 선택(`git checkout feature -- file`)했다. 기능 브랜치는 main보다 훨씬 이전 시점에서 분기됐기 때문에 그 사이 main에 추가된 개선사항(버튼 스타일, 키컬러 UI, 로고 설정, 미리보기 크기 조절 등)이 모두 덮어씌워졌다. 이 세션에서 대부분 복원 완료했으나, `gmbf/po-c/index.html`의 admin-link 한 항목이 미복원 상태다.

**이후 이어갈 작업 (기능 개발):**
- CMS 블록 에디터 추가 블록 유형 (임베드형 완성, 구분선 등)
- 셀러 포털 gmbf/po-c의 스테이지 내용을 CMS 블록으로 연결
- 관리자 페이지 셀러 명단 관리 기능
