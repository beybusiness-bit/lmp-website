...
9. **다음 세션 시작용 복붙 블록 출력** — 테스트 체크리스트 + 다음 세션 프롬프트를 **하나의 블록**으로 합쳐서 출력한다. 사용자가 통째로 복사해서 다음 세션 첫 메시지로 붙여넣을 수 있어야 한다.

```
🧪 이번 배포 테스트 체크리스트
[ ] 기능명 — 어디를 열고, 무엇을 클릭/입력하고, 무엇이 보여야 하는지를 쉬운 말로
[ ] 기능명 — (전문 용어 금지. 사용자가 직접 따라할 수 있는 수준으로)
피드백은 이 메시지 아래에 적어서 다음 세션 첫 메시지로 붙여넣어 주세요.

---
[다음 세션 프롬프트]
환경 확인: Remote(☁️)인지 Local(💻)인지 먼저 판별해줘.
Remote라면 시스템이 자동 생성한 feature 브랜치(claude/...)에서 작업하고,
배포 시점에 mcp__github__create_pull_request + mcp__github__merge_pull_request로 main에 머지하는 방식이야.
git push origin main 직접 시도는 403으로 거부되니까 절대 시도하지 말아줘.

이번 세션 작업: [다음에 할 작업 내용]
```
