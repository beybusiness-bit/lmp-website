...
6. **테스트 체크리스트 출력** — 이번 세션에서 변경된 기능 중 사용자가 직접 확인해야 할 항목을 아래 형식으로 정리:
   ```
   🧪 이번 배포 테스트 체크리스트
   [ ] 기능명 — 확인 방법 (어디서, 무엇을 클릭/입력)
   [ ] 기능명 — 확인 방법
   ...
   피드백은 다음 세션 시작 시 알려주세요.
   ```
7. CLAUDE.md 직접 갱신 (완료 단계 ✅, 다음 시작점 업데이트)
8. CLAUDE.md 갱신분도 동일 절차로 push + PR 머지 (CLAUDE.md 갱신 = 세션 마무리의 일부이므로 별도 PR 불필요 — 같은 PR에 포함하거나 바로 이어서 1번 더 머지)
9. PAT 설정된 경우 현재 PAT 출력
